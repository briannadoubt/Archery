...
public struct MigrationRunner: Sendable {
    ...
    public func run(on container: PersistenceContainer) throws {
    ...
    /// Run all pending migrations on a PersistenceWriter directly
    public func run(on writer: any GRDB.DatabaseWriter) throws {
        var migrator = DatabaseMigrator()
    ...
    public func hasPendingMigrations(on writer: any GRDB.DatabaseWriter) throws -> Bool {
        var migrator = DatabaseMigrator()
        for migration in migrations {
...
public struct MigrationBuilder {
    ...
    public static func buildArray(_ components: [[Migration]]) -> [Migration] {
        components.flatMap { $0 }
    }

    public static func buildOptional(_ component: [Migration]?) -> [Migration] {
        component ?? []
    }

    public static func buildEither(first component: [Migration]) -> [Migration] {
        component
    }

    public static func buildEither(second component: [Migration]) -> [Migration] {
        component
    }
}

public extension MigrationRunner {
    /// Create a migration runner using a result builder
    init(@MigrationBuilder _ builder: () -> [Migration]) {
...
public extension Migration {
    ...
    static func dropTable(id: String, table: String) -> Migration {
        Migration(id: id) { db in
    ...
    static func sql(id: String, _ sql: String) -> Migration {
        Migration(id: id) { db in
